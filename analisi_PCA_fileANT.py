...
data = pd.read_csv('testrespiroseduto.txt', sep=",|:", header=None, engine='python')
...
lowThreshold_A = max(0.05, (fStimMean_A-fStimstd_A))  # creation of the abdomen threshold
...
lowThreshold_T = max(0.05, (fStimMean_T-fStimstd_T))  # creation of the thorax threshold
...
f1 = max(0.05, fBspectrum_A-0.4)
f2 = fBspectrum_A+0.4

ft_pl = f2
Wn_pl = ft_pl/(fdev/2)
b, a = scipy.signal.butter(1, Wn_pl, 'lowpass')  # low pass filter (Butterworth)
...
for i in range(len(Min_Ind_A)):
    ...
    ti_te = ti/te
    TiTe_A.append(ti/te)
    if i != len(Min_Ind_A)-1:
        ttot = (Min_Ind_A[i + 1] - Min_Ind_A[i]) / fdev
        fb = 1 / ttot * 60
        T_A.append(ttot)
        fB_A.append(fb)
...
for i in range(len(Min_Ind_T)):
    ...
    ti_te = ti/te
    TiTe_T.append(ti/te)
    if i != len(Min_Ind_T)-1:
        ttot = (Min_Ind_T[i + 1] - Min_Ind_T[i]) / fdev
        fb = 1 / ttot * 60
        T_T.append(ttot)
        fB_T.append(fb)
...
for i in range(len(Min_Ind_Tot)):
    ...
    if i != len(Min_Ind_Tot)-1:
        ttot = (Min_Ind_Tot[i + 1] - Min_Ind_Tot[i]) / fdev
        fb = 1 / ttot * 60
        T_Tot.append(ttot)
        fB_Tot.append(fb)
